...
balanceArray = [255, 455, 385, 759, 500, 284, 581, 684, 255, 455, 385, 759, 500, 293, 585, 342, 684];
$(document).ready(function() {
    ...
    $(".table-user .icon .btn").click(function(e) {
        ...
        var t = $(this),
        n = t.parents("tr"),
        r = n.find("td").eq(1).text(),
        i = n.find("td").eq(0).find("img").attr("src"),
    ...
    if ($("#online-users").length > 0) {
        ...
        function() {
            $("#online-users").show().sparkline(getUser(), {
                width: e.width() > 200 ? 100: e.width() - 100,
                height: "25px",
                enableTagOptions: !0
            });
            $("#balance").show().sparkline(getBalance(), {
                height: "25px",
                barWidth: e.width() > 200 ? 4: Math.floor((e.width() - 100) / 17) - 1,
                enableTagOptions: !0
            })
        })
    }
    var t = [];
    for (var n = 0; n <= 20; n += 1) t.push([n, parseInt(32 + Math.random() * 35)]);
    var r = [];
    for (var n = 0; n <= 20; n += 1) r.push([n, parseInt(32 + Math.random() * 55)]);
    var i = [];
    for (var n = 0; n <= 10; n += 1) i.push([n, parseInt(31 + Math.random() * 7)]);
    var s = new Array;
    s.push({
    ...
    if ($("#flot-3").length > 0) {
        ...
        function g() {
            ...
            while (f.length < m) {
                var e = f.length > 0 ? f[f.length - 1] : 50,
    ...
    if ($("#flot-audience").length > 0) {
        var f = [[1262304e6, 1300], [12649824e5, 2200], [12674016e5, 3600], [127008e7, 5200], [1272672e6, 4500], [12753504e5, 3900], [12779424e5, 3600], [12806208e5, 4600], [12832992e5, 5300], [12858912e5, 7100], [12885696e5, 7800], [12912417e5, 8195]];
        $.plot($("#flot-audience"), [{
            label: "Visits",
            data: f,
            color: "#3a8ce5"
    ...
    if ($("#flot-hdd").length > 0) {
        var f = [[1364598e6, 10], [13646016e5, 12], [13646052e5, 14], [13646088e5, 14], [13646124e5, 10], [1364616e6, 16], [13646196e5, 18], [13646232e5, 15], [13646268e5, 16], [13646304e5, 18], [1364634e6, 20], [13646376e5, 22], [13646412e5, 24], [13646448e5, 25], [13646484e5, 27], [1364652e6, 31], [13646556e5, 33], [13646592e5, 36], [13646628e5, 37], [13646664e5, 38], [136467e7, 39], [13646736e5, 42], [13646772e5, 45], [13646808e5, 47], [13646844e5, 50]];
        $.plot($("#flot-hdd"), [{
            label: "HDD usage",
            data: f,
            color: "#f36b6b"
    ...
    if ($(".calendar").length > 0) {
        ...
        $(".calendar").fullCalendar("addEventSource", [{
            title: "All Day Event",
            start: new Date(k, C, 1)
        },
        {
            title: "Long Event",
            start: new Date(k, C, N - 5),
            end: new Date(k, C, N - 2)
        },
        {
            id: 999,
            title: "Repeating Event",
            start: new Date(k, C, N - 3, 16, 0),
            allDay: !1
        },
        {
            id: 999,
            title: "Repeating Event",
            start: new Date(k, C, N + 4, 16, 0),
            allDay: !1
        },
        {
            title: "Meeting",
            start: new Date(k, C, N, 10, 30),
            allDay: !1
        },
        {
            title: "Lunch",
            start: new Date(k, C, N, 12, 0),
            end: new Date(k, C, N, 14, 0),
            allDay: !1
        },
        {
            title: "Birthday Party",
            start: new Date(k, C, N + 1, 19, 0),
            end: new Date(k, C, N + 1, 22, 30),
            allDay: !1
        },
        {
            title: "Click for Google",
            start: new Date(k, C, 28),
            end: new Date(k, C, 29),
            url: "http://google.com/"
        }])
    }
    ...
    if ((location.pathname == "/flat/" || location.pathname == "/flat" || location.pathname == "/flat/index.html") && location.host != "localhost" && $(window).width() > 767) {
        bootbox.animate(!1);
        bootbox.confirm("Would you like to start the page guide? It will show you functions you could miss!", "No", "Yes", 
        function(e) {
            e && $.pageguide(L, {
                events: {
                    close: function() {
    ...
    if ($("#simg").length > 0) {
        function O(e) {
            return e.id ? "<img style='padding-right:10px;' src='adminskin/img/demo/flags/" + e.id.toLowerCase() + ".gif'/>" + e.text: e.text
        }
        $("#simg").select2({
            formatResult: O,
            formatSelection: O,
    ...
    if ($("#map2").length > 0) {
        ...
        function B(e) {
            ...
            $("#map2").gmap3({
                marker: {
                    ...
                    options: {
                        draggable: !0,
                        icon: new google.maps.MarkerImage("http://maps.gstatic.com/mapfiles/icon_green" + (e ? "A": "B") + ".png")
                    },
                    tag: e ? "from": "to",
                    events: {
                        dragend: function(t) {
        ...
        M.add("Direction to here", "itemB", 
        function() {
            M.close();
            B(!1)
        });
        M.add("Direction from here", "itemA separator", 
        function() {
            M.close();
            B(!0)
        });
        M.add("Zoom in", "zoomIn", 
        function() {
            var e = $("#map2").gmap3("get");
            e.setZoom(e.getZoom() + 1);
            M.close()
        });
        M.add("Zoom out", "zoomOut", 
        function() {
            var e = $("#map2").gmap3("get");
            e.setZoom(e.getZoom() - 1);
            M.close()
        });
        M.add("Center here", "centerHere", 
        function() {
